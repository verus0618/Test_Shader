...
public class WallPassRuntime : MonoBehaviour
{
    ...
    {
        if (r != null && !_receivers.Contains(r))
        {
            _receivers.Add(r);
        }
    }
    ...
    public void Emit(Vector3 a, Vector3 b, float radius)
    {
        ...
        {
            var r = _receivers[i];
            if (r == null || r.Space != WallPassReceiver.MaskSpace.Anchored)
            {
                continue;
            }
            if (!OverlapsBounds(r.WorldBounds, a, b, radius))
            {
                continue;
            }
    ...
    public bool OverlapsAnyReceiver(Vector3 a, Vector3 b, float radius)
    {
        ...
        {
            return true; // no walls registered, do not filter
        }

        for (int i = 0; i < _receivers.Count; i++)
        {
            var r = _receivers[i];
            if (r != null && OverlapsBounds(r.WorldBounds, a, b, radius))
            {
                return true;
            }
        }
